...
class MessagesUtilSpec extends PlaySpec with MockitoSugar with ScalaFutures {

  lazy val mockRepo: MongoMessageRepository = mock[MongoMessageRepository]
  lazy val mockAuthConnector: AuthIdentifiersConnector = mock[AuthIdentifiersConnector]
  ...
  lazy val mockPreferencesConnector: PreferencesConnector = mock[PreferencesConnector]
  lazy val mockEnrolmentProxyConnector: EnrolmentProxyConnector = mock[EnrolmentProxyConnector]
  lazy val mockTaxpayerNameConnector: TaxpayerNameConnector = mock[TaxpayerNameConnector]
  implicit val hc: HeaderCarrier = HeaderCarrier()

  val responseAsJson: (String, String) => ContentParameters = (reason: String, failureId: String) =>
  ...
  private val injector: Injector = new GuiceApplicationBuilder()
    ...
    .configure(
      ...
      "scheduling.externalMessageAdapterCleanup.taskEnabled" -> false
    )
    .injector()
  ...
  "checkActiveUsers" must {

    "check active users, not validate preferences when enrolment given is HMRC-IOSS-INT HMRC-IOSS-NETP" in new TestCase() {
      when(mockEnrolmentProxyConnector.hasActiveUsers(any[List[String]], any[String])(any[HeaderCarrier]))
      ...
      val message1: Message = MessageFixtures
        ...
        .copy(recipient = TaxEntity(Regime.ioss, HmrcIossInt("IN9001234567"), None))

      val message2: Message = MessageFixtures
        .testMessageWithoutContent(recipientId = utr)
        .copy(recipient = TaxEntity(Regime.ioss, HmrcIossInt("IM9001234567"), None))

      val request: Request[JsValue] = FakeRequest().withBody(Json.obj())
      val hc = HeaderCarrier()
      val result1 = messagesUtil.checkActiveUsers(Users(List("123456", "789012")), message1, "enrolment")(request, hc)
      val result2 = messagesUtil.checkActiveUsers(Users(List("123456", "789012")), message2, "enrolment")(request, hc)
  ...
  "auditMessageDeliveryStatus" must {
    "audit the event and return correct audit result" in new TestCase() {
      ...
      val result = await(messagesUtil.auditMessageDeliveryStatus(eventHubEvent))
